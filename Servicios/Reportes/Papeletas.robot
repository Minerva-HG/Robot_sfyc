*** Settings ***
Documentation    Reporte papeletas
Library    String
...
${Reporte_papeletas}    xpath=(//td[contains(.,'Papeletas')])[1]
###############################################Combos##########################################################################################
...
Fechas
    [Documentation]    Reporte papeletas
    [Tags]    "Validará que pueda despeglar el calendario de la lista despegable ""Desde"" y ""Hasta"". Validará que la fecha hasta no sea menor que fecha desde."
    Fecha desde
    Fecha hasta

Papeletas
    [Documentation]    Reporte papeletas
    [Tags]    "Valida que habilite el botón aceptar, y que genere el reporte "
    Test boton aceptar

Boton Limpiar
    [Documentation]    Reporte papeletas
    [Tags]    "Valida que restablezca los datos a como estaban en un principio"
    Test boton limpiar

Boton Cancelar
    [Documentation]    Reporte papeletas
    [Tags]    "Valida que el sistema lo dirija a la pantalla anterior en la que estaba"
    Test boton cancelar
...
Validacion de usuarios
    ...
    FOR    ${counter}    IN RANGE    1     2
        ...
        Sleep    4s
        Click Element    ${Reporte_papeletas}
    #IF    ${counter} <= ${7}
        #Sleep    10s
        #Close Browser
    #END
