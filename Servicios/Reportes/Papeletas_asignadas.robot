*** Settings ***
Documentation    Reporte papeletas asignadas
Library    String
...
${Reporte_papeletas_asignadas}    xpath=//td[contains(.,'Papeletas asignadas')]
######################################Botones#############################################################################################################################
...
#########################################Checkbox todos###########################################################################
${Checkbox_todos}    xpath=//span[contains(@class,'dx-checkbox-icon')]
########################################Combos###########################################################################################
${Combo_del_vendedor}    xpath=/html/body/app-root/app-side-nav-outer-toolbar/dx-drawer/div/div[2]/dx-scroll-view/div[1]/div/div[1]/div[2]/div/app-reporting-main-container/app-shared-reporting-main-container/div/div/div[1]/div[3]/app-reporting-ballots-assigned-form/form/app-reporting-salesman-parameter/div/div/div/div[2]/app-salesman-dropdown[1]/dx-drop-down-box/div[1]/div/div[1]/input
${Combo_al_vendedor}    xpath=/html/body/app-root/app-side-nav-outer-toolbar/dx-drawer/div/div[2]/dx-scroll-view/div[1]/div/div[1]/div[2]/div/app-reporting-main-container/app-shared-reporting-main-container/div/div/div[1]/div[3]/app-reporting-ballots-assigned-form/form/app-reporting-salesman-parameter/div/div/div/div[2]/app-salesman-dropdown[2]/dx-drop-down-box/div/div/div[1]/input
########################################Vendedor######################################################################################################
${Del_vendedor}    xpath=//td[contains(.,'LIZARRAGA ROSALES JUAN ALFONSO')]
${Al_vendedor}    xpath=(//td[contains(.,'LIZARRAGA ROSALES JUAN ALFONSO')])[2]
...
Reporte con todos los vendedores
    [Documentation]    Reporte papeletas asignadas
    [Tags]    Reporte papeletas asignadas con todos los vendedores
    Todos los vendores

Combos
    [Documentation]    Reporte papeletas asignadas
    [Tags]    Validacion de Combos
    Checkbox todos
    Combo del vendedor
    Combo al vendedor
...
Papeletas asignadas
    [Documentation]    Reporte papeletas asignadas
    [Tags]    Validación para generar reporte
    Test boton aceptar
...
Boton limpiar
    [Documentation]    Reporte papeletas asignadas
    [Tags]    Validación boton limpiar
    Tets boton limpiar

Boton cancelar
    [Documentation]    Reporte papeletas asignadas
    [Tags]    Validación boton cancelar
    Test boton cancelar
...
Validacion de usuarios
    ...
    FOR    ${counter}    IN RANGE    1     2
        ...
        Sleep    4s
        Click Element    ${Reporte_papeletas_asignadas}
    #IF    ${counter} <= ${7}
        #Sleep    10s
        #Close Browser
    #END
